...
- If the plan entrypoint contains `## Research Context`, a `Source:` / `Reference:` line pointing to `RESEARCH.md`, or any path/link to the resolved `paths.research` artifact, treat the Research Context embedded in the plan as the committed requirements snapshot. Extract and read the exact `RESEARCH.md` source path from that line; fall back to the configured `paths.research` only for a legacy link without a usable path. This supports a legacy research file and `<research-bundle>/RESEARCH.md` without treating sibling C4/ADR/dependency files as extra committed requirements. Read the source only to verify the committed revision marker (`Updated:` and/or `SHA256:`) and to consult `## Sessions` for rationale when needed. If the source line lacks a revision marker, the source is missing, or the current `Active Summary` revision differs, emit `WARN [research-drift]` and verify against the plan's embedded Research Context; do not fail or expand scope based on newer live research unless the user explicitly asks to rebase/refine the plan. Skipping this drift check is a verification bug.
