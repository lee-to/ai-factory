...
- Confirming a marked critical turns the **internal pre-validation draft** from `warn` into a published `fail`: the marker is gone, so the item counts as a normal blocker and `suggested_next.command` resolves to `/aif-fix`. The `warn` plus `command: null` pair is a draft-only state — the published gate is already marker-free and never carries it
