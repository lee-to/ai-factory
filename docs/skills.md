...
Browser/UI automation is saved as one dependency-free Playwright-compatible script per case under `paths.qa/<branch-slug>/browser-replay/TC-NNN.js`. On later agent runs, matching scripts are executed before exploratory browser actions; after a revision, worktree, or manual build change, scripts for both prior passes and failures are replayed to verify the fix and catch regressions. Scripts are reused while their embedded case digest matches, and credentials, tokens, cookies, and token-bearing URLs are never persisted in them.
