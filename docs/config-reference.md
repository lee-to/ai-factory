...
| `workflow.plan_structure` | `classic` | `/aif-plan`, `/aif-implement` | Plan commit layout: `classic` uses a separate `## Commit Plan` section; `task-based` writes explicit commit tasks inline in the task list. |
