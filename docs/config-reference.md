...
| `workflow.plan_id_format` | `slug` | `/aif-plan`, `/aif-implement`, `/aif-improve`, `/aif-explore`, `/aif-verify`, `/aif-rules-check` | Active values: `slug` (default), `sequential`. Reserved values (currently fall back to `slug` with an `INFO` log): `timestamp`, `uuid`. `sequential` writes `paths.plans/<NNNN>_<stem>.md` where `stem` is the canonical Handoff/branch/slug stem from `/aif-plan` Step 1.2.a. Numbering is monotonic, zero-padded to 4 digits, bounded at `9999` (the producer errors before writing `10000_…`). Force-disabled when `HANDOFF_BRANCH_PREPARED=1`. |
