...
- `null` means only that no allowlisted command fits the current state. It carries no verdict of its own: whether the run may proceed is decided by `status` and `blocking` alone, and orchestrators read those, never the command. `/aif-review` does **not** use `null` to encode an unresolved finding: a review that produced `(confidence: low)` / `(confidence: medium)` markers runs its validation pass before the gate is computed, so a published block only ever reports states schema v1 can express — a confirmed critical is an ordinary blocker, a refuted one is gone. When that validation fails, the review reports the failure as a `fail` gate with a `review-validation-failed` blocker; `null` appears there because the recovery (re-running `/aif-review +check`) is not an allowlisted command, and the reason says so.
