...
- `null` means no allowlisted command fits the current state — the run is not cleared to proceed, but the next step is not one of the gate commands either. Orchestrators must not treat a `null` command as "nothing left to do". `/aif-review` does **not** use `null` to encode an unresolved finding: a review that produced `(confidence: low)` / `(confidence: medium)` markers runs its validation pass before the gate is computed, so a published block only ever reports states schema v1 can express — a confirmed critical is an ordinary blocker, a refuted one is gone. The single exception is a validator dispatch failure, which keeps the pre-validation gate and prints a `WARN [+check]` line.
