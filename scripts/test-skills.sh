...
# skills_dir patterns
HARDCODED_SKILLS_DIR=$(grep -rE '\.(claude|cursor|codex|github|gemini|junie|qwen|windsurf|devin|warp)/skills' "$ROOT_DIR/skills/" "$ROOT_DIR/subagents/" --include='*.md' 2>/dev/null | grep -v '{{' | wc -l | tr -d ' ' || true)
if [[ "$HARDCODED_SKILLS_DIR" -eq 0 ]]; then
    ...
    grep -rEn '\.(claude|cursor|codex|github|gemini|junie|qwen|windsurf|devin|warp)/skills' "$ROOT_DIR/skills/" "$ROOT_DIR/subagents/" --include='*.md' 2>/dev/null | grep -v '{{' | sed 's/^/      /'
fi
...
HARDCODED_AGENT_FLAG=$(grep -rE '--agent (claude-code|cursor|codex|github-copilot|gemini-cli|junie|windsurf|devin)' "$ROOT_DIR/skills/" "$ROOT_DIR/subagents/" --include='*.md' 2>/dev/null | grep -v '{{' | wc -l | tr -d ' ' || true)
if [[ "$HARDCODED_AGENT_FLAG" -eq 0 ]]; then
    ...
    grep -rEn '--agent (claude-code|cursor|codex|github-copilot|gemini-cli|junie|windsurf|devin)' "$ROOT_DIR/skills/" "$ROOT_DIR/subagents/" --include='*.md' 2>/dev/null | grep -v '{{' | sed 's/^/      /'
fi
